...
def get_doge_data():
    try:
        ...
        logger.error(f"[DOGE] Veri çekme hatası: {str(e)}")
        return pd.DataFrame()
...
def preprocess_doge(df):
    try:
        ...
        logger.error(f"[DOGE] Özellik çıkarma hatası: {str(e)}")
        return pd.DataFrame()
...
def predict_doge_next_day():
    try:
        ...
        if df.empty or len(df) < 15:
            logger.warning("[DOGE] Yeterli veri yok, tahmin yapılmadı.")
            return
        ...
        logger.info(f"[DOGE] Tahmin yapıldı - Yön: {direction} - Tarih: {prediction_date}")
        ...
        db.commit()
        logger.info("[DOGE] Tahmin veritabanına kaydedildi.")
    except Exception as e:
        logger.error(f"[DOGE] Tahmin sırasında hata: {str(e)}")
