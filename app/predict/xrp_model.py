...
def predict_xrp_next_day():
    ...
    print(f"[XRP] Tahmin edilen yön: {'Artış' if prediction == 1 else 'Azalış'} - Tarih: {prediction_date}")

    db = next(get_db())
    prediction_record = CoinPrediction(
        coin_id=4,  # XRP için ID
        predicted_price=0.0,
        predict=prediction,
    ...
    db.commit()
    print("XRP tahmini veritabanına kaydedildi.")
