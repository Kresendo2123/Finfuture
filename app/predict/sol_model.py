...
def predict_sol_next_day():
    ...
    print(f"[SOL] Tahmin edilen yön: {'Artış' if prediction == 1 else 'Azalış'} - Tarih: {prediction_date}")

    db = next(get_db())
    prediction_record = CoinPrediction(
        coin_id=6,  # SOL için ID
        predicted_price=0.0,
        predict=prediction,
    ...
    db.commit()
    print("SOL tahmini veritabanına kaydedildi.")
